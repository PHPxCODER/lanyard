...
Lanyard is a service that makes it super easy to export your live Discord presence to an API endpoint (`phpxcoder.rest/v1/users/:your_id`) and to a WebSocket (see below) for you to use wherever you want - for example, I use this to display what I'm listening to on Spotify on my personal website. It also acts as a globally-accessible realtime KV store which you can update from the Lanyard Discord bot or from the Lanyard API.
...
Just [join this Discord server](https://discord.gg/d4jpsFq76f) and your presence will start showing up when you `GET phpxcoder.rest/v1/users/:your_id`. It's that easy.
...
[phpxcoder.in](https://phpxcoder.in) - Vue JS Portfolio Page which Uses API for Discord & Spotify Presence from Lanyard.
[v2.phpxcoder.in](https://v2.phpxcoder.in) - Next JS Portfolio Page which Uses API for Discord Presence from Lanyard.
[v3.phpxcoder.in](https://v3.phpxcoder.in) - Svelte Portfolio Page which Uses API for Discord Presence from Lanyard.
...
`GET https://phpxcoder.rest/v1/users/:user_id`
...
```js
{
  ...
  "data": {
    ...
    "discord_user": {
      ...
      "id": "697757845063729194",
      "discriminator": "6969",
      "avatar": "a_7f880055f337744dc473a0192bbc1b45"
    },
...
DM the Lanyard bot (`Lanyard#8228`) with `.apikey` to get your API key.
...
`PUT https://phpxcoder.rest/v1/users/:user_id/kv/:key`
...
`PATCH https://phpxcoder.rest/v1/users/:user_id/kv`
...
`DELETE https://phpxcoder.rest/v1/users/:user_id/kv/:key`

## Socket Docs

The websocket is available at `wss://phpxcoder.rest/socket`. If you would like to use compression, please specify `?compression=zlib_json` at the end of the URL.
...
```js
{
  ...
  d: {
    ...
    subscribe_to_ids: ["697757845063729194"]
  }
}
```
...
`https://phpxcoder.rest/<id>.<file_type>`
...
docker build -t phpxcoder/lanyard:latest .
```
...
```bash
docker run --rm -it -p 4001:4001 -e REDIS_HOST=redis -e BOT_TOKEN=<token> --link lanyard-redis:redis phpxcoder/lanyard:latest
```
...
services:
  ...
  lanyard:
    image: phpxcoder/lanyard:latest
    restart: always
...
- [phpxcoder.in](https://phpxcoder.in)
